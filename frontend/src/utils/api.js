...
export const updateUserAvatar = (avatarFile) => {
  ...
  if (!(avatarFile instanceof File || avatarFile instanceof Blob)) {
    return Promise.reject(new Error('Expected File or Blob object for avatar'));
  }

  const formData = new FormData();
  formData.append('avatar', avatarFile, avatarFile.name); // Добавляем имя файла

  return fetch(`${URL}/users/me/avatar`, {
    method: 'PATCH',
    headers: {
      authorization: `Bearer ${sessionStorage.getItem("auth_token")}`,
      // Явно не устанавливаем Content-Type!
    },
    body: formData,
  })
  .then(checkResponse)
  .then(data => {
    // Можно добавить дополнительную обработку ответа
    return data;
  })
  .catch(error => {
